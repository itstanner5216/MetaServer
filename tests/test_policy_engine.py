...
async def test_policy_decision_includes_reason():
    ...
    assert "read_only" in decision.reason.lower() or "blocked" in decision.reason.lower()
    ...
    for mode in modes:
        for risk in risks:
            ...
            assert decision.action in ["allow", "block", "require_approval"], (
                f"Invalid action for {mode} + {risk}: {decision.action}"
            )
...
async def test_unknown_risk_level_fails_safe():
    ...
    assert decision.action in ["block", "require_approval"], (
        f"Unknown risk should fail-safe, got: {decision.action}"
    )
