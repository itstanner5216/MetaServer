...
def detect_agent_id(ctx: Context) -> str | None:
    ...
    # Strategy 4: Future enhancement for Redis-backed session mapping
...
async def get_agent_id_for_session(session_id: str) -> str | None:
    """
    Stub: returns None. Redis-backed session→agent mapping is a planned enhancement.
...
async def set_agent_id_for_session(session_id: str, agent_id: str, ttl: int = 3600) -> bool:
    """
    Stub: returns False. Redis-backed session→agent mapping is a planned enhancement.
